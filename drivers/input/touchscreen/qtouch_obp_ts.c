			      ...
			      void *_msg)
{
	...
	if (qtouch_resume_cal) {
		if (msg->status&0x40) {
			...
			/*
			pr_info("%s: [Increase] stat=%02x,
				f=%d qtouch_num_touch_cnt=%d\n", __func__,
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (err == 0) {
		...
		pr_err("%s: Failed reading info block - 0 %i\n",
			__func__, err);
	...
	if ((ts->touch_fw_size != 0) && (ts->touch_fw_image != NULL)) {
		...
		if (err != ts->boot_pkt_size) {
			...
			err = qtouch_process_info_block(ts);
			if (err) {
				pr_err("%s: Failed reading info block - 1 %i\n",
				__func__, err);
				goto err_reading_info_block;
			}
			goto err_boot_mode_failure;
